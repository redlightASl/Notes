...
27. 三极管四种情况下的少子分布，临界饱和条件
     ...
     *临界饱和状态下，从基极抽取的电子和集电区本身空穴浓度恰好达到平衡，集电区载流子分布与平衡时载流子分布一样，是处处相同的*，载流子分布如下图所示
...
36. Gummel数的意义
     ...
     缓变基区晶体管基区中多子的总量即Gummel数$Q_{GB}$

37. 基区扩展电阻效应（发射极电流集边效应）
...
41. Kirk效应（基区展宽效应）
     ...
     随着集电极电流增大，从集电区进入基区的空穴增多，表现出基区展宽的现象
     $$
     J_{kirk}=qv_S N_C,n=\frac{J_C}{qv_s}
     $$
...
42. **异质结晶体管（HBT）的优点和特点**
     ...
     * 基区不容易穿通，从而厚度可以做小
     * $R_B$降低，$f_{max}$可提高
     * 基区电导调制效应、大注入效应都不明显，可以改善大注入导致的增益$\beta$下降
...
43. 理想MOS二极管以及能带图正偏反偏
     ...
     * $\psi_s \ge 2\psi_B$：强反型。半导体表面电子浓度等于甚至大于体内空穴浓度
     ...
     表面电荷：
     $$
     Q_{sc}=-\sqrt{1q\epsilon_s N_A (2\psi_{FP})}
     $$
     氧化层电容：等于氧化层介电常数与厚度之比
     $$
...
45. 阈值电压表达式是什么？如何调整阈值电压？
     ...
     **考虑金属半导体功函数差后的阈值电压**：
     $$
     V_{TH}=V_{FB}+V_{T0}=(\phi_{ms}-\frac{Q_0}{C_0})+\phi_s -\frac{Q_B}{C_{ox}}=\phi_{ms}-\frac{Q_0}{C_0}-\frac{Q_B}{C_{ox}} +\phi_s
     $$
     离子注入和衬底偏压都可以改变阈值电压

46. 金属和半导体存在功函数差的时候的能带图

     平衡情况下，半导体表面电势不为0，需要在金属电极上施加一个电压来消除金属半导体功函数差的影响，该电压称为**平带电压**
     $$
     V_{FB}=\phi_m -\phi_s
     $$
